...
import { AlertCircle, Tool, FileText, CheckCircle, Clock } from 'lucide-react';
      ...
      <div className="main-content">
        ...
        <div className="filters-bar" style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          ...
          <style>{`
            ...
            .incidencia-header { padding: 1rem 1.5rem; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center; background: #fafafa; }
            ...
            filtered.map(inc => (
              <div key={inc.id} className="incidencia-card">
                <div className="incidencia-header">
                  <div>
                    <h3 style={{ margin: 0, color: '#0A2342' }}>{inc.Mantenimientos?.Instalaciones?.Clientes_Mant?.razon_social}</h3>
                    <div style={{ fontSize: '0.85rem', color: '#666', marginTop: '4px' }}>
                      Ref. Mantenimiento: {inc.id_mantenimiento.split('-')[0].toUpperCase()} ({new Date(inc.created_at).toLocaleDateString()})
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span className="pill" style={{ backgroundColor: inc.estado === 'Cerrada' ? '#28a745' : '#E63329', color: 'white' }}>
                      {inc.estado}
                    </span>
                  </div>
                </div>
